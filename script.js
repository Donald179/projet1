const products = [
  ...
  `).join('');
    ...
    function orderWA(id) {
      const p = products.find(x => x.id === id);
      window.open(`https://wa.me/22670000000?text=Bonjour%2C%20je%20veux%20commander%20*${encodeURIComponent(p.name)}*%20à%20${encodeURIComponent(p.price)}%20FCFA`, '_blank');
    }
    ...
    function sendCartWA() {
      ...
      window.open(`https://wa.me/22670000000?text=Bonjour%2C%20voici%20ma%20commande%20:%0A%0A${lines}%0A%0ATotal%20:%20${total}%20FCFA`, '_blank');
    }
